...
int eraser_events(sfRenderWindow *window, sfEvent *event, event_t *events)
{
    while (sfRenderWindow_pollEvent(window, event)) {
        ...
        if (events->if_pressed)
            erase_options(window, events->if_pressed, events->colour,
            events->color);
    }
    return 0;
}
    ...
    color_t *color)
{
    ...
    if (if_pressed && (position.x >= 280 && position.x <= 1917 &&
        ...
        circle = sfCircleShape_create();
            sfCircleShape_setRadius(circle, 8);
...
void eraser_tool(sfRenderWindow *window, color_t *color)
{
    ...
    events.if_pressed = sfFalse;
    events.colour = sfWhite;
    events.color = color;
    while (sfRenderWindow_isOpen(window))
        eraser_events(window, &event, &events);
}
